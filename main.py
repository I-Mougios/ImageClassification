...
def train_evaluation_knn_classifier(train_features, train_labels,test_features,test_labels, weights='uniform'):
    ...
    evaluation_reports = (confusion_matrix(test_labels,predictions),
...
superclass_dict = {superclass_num: superclass_name
                   for superclass_num, superclass_name
                   in zip(superclass_number, superclass_names)}
...
# Fitting the model
autoencoder.fit(X_train[:4501], X_train[:4501], epochs=70, batch_size=8,
                validation_data=(X_train[4501:], X_train[4501:]), callbacks=[early_stop])
...
sns.lineplot(data=autoencoder_loss, x=autoencoder_loss.index.astype(int),
             y=autoencoder_loss['loss'], label='loss')
sns.lineplot(data=autoencoder_loss, x=autoencoder_loss.index.astype(int),
             y=autoencoder_loss['val_loss'], label='val loss')
plt.xlabel('Epochs')
plt.ylabel('Loss')
...
knn_raw_images = train_evaluation_knn_classifier(train_features=X_train, train_labels=y_train, test_features=X_test, test_labels=y_test,)
print('Confusion Matrx: \n', knn_raw_images[1][0])
print('Classification Report: \n', knn_raw_images[1][1], '\n'*3)

knn_encoded_images = train_evaluation_knn_classifier(train_features=encoded_features_train, train_labels=y_train, test_features= encoded_features_test, test_labels=y_test,)
print('Confusion Matrx: \n',knn_raw_images[1][0])
print('Classification Report: \n',knn_raw_images[1][1], '\n'*3)

knn_image_embeddings = train_evaluation_knn_classifier(train_features=images_embeddings_train, train_labels=y_train, test_features=images_embeddings_test, test_labels=y_test)
...
images_embeddings_train_normalized = images_embeddings_train / np.linalg.norm(images_embeddings_train, axis=1, keepdims=True)
image_embeddings_test_normalized = images_embeddings_test / np.linalg.norm(images_embeddings_test, axis=1, keepdims=True)
